-- Comprehensive fix for business save constraint violation

-- Step 1: Temporarily disable the constraint (if it exists)
ALTER TABLE public.memberships DROP CONSTRAINT IF EXISTS memberships_role_check;

-- Step 2: Fix any invalid roles in the memberships table
UPDATE public.memberships
SET role = CASE
  WHEN role = 'merchant' THEN 'trusted_partner'
  WHEN role = 'user' THEN 'member'
  WHEN role NOT IN ('admin', 'trusted_partner', 'member') THEN 'member'
  ELSE role
END;

-- Step 3: Update the complete_business_profile function to use correct terminology
...
-- Step 4: Re-enable the constraint
