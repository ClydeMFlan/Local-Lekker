...
class _TrustedPartnerHomePageState extends State<TrustedPartnerHomePage>
  ...
  Widget _buildDesktopTrustedPartnerHeroCard() {
    ...
    final IconData bankingIcon = _hasActiveBanking
        ? Icons.verified_user_rounded
        : Icons.account_balance_wallet_outlined;
    final String bankingLabel = _hasActiveBanking
        ? 'Banking Details Active'
        : 'Add Banking Details';
    final String bankingDescription = _hasActiveBanking
        ? (_bankName?.trim().isNotEmpty == true
            ? '${_bankName!.trim()} connected and ready for payouts'
            : 'Your payout setup is active and ready')
        : 'Finish your payout setup so member payments can settle correctly';

    return LayoutBuilder(
      builder: (context, constraints) {
        final bool stackVertically = constraints.maxWidth < 860;
        final bool isPortrait = MediaQuery.of(context).orientation == Orientation.portrait;
        final bool useLogoWidthActionStack = stackVertically && isPortrait;
        final bool useCompactScannerCard = useLogoWidthActionStack;
        final int gridColumns = 12;
        final int leftPanelColumns = 7;
        const int rightPanelColumns = 4;
        final double outerCardTopPadding = useLogoWidthActionStack ? 0 : (isCompactHeight ? 12 : 28);
        final double outerCardSidePadding = isCompactHeight ? 12 : 28;
        final double outerCardBottomPadding = isCompactHeight ? 12 : 28;
        final double identityPanelPadding =
          (useLogoWidthActionStack && isCompactHeight)
          ? 10
          : (isCompactHeight ? 14 : 24);
        final double scannerPanelPadding = useCompactScannerCard ? 4 : (isCompactHeight ? 8 : 18);
        final double logoSize =
          (useLogoWidthActionStack && isCompactHeight)
          ? 82
          : (isCompactHeight ? 90 : 122);
        final double logoBorderThickness = isCompactHeight ? 4 : 5;
        final double rowGap = isCompactHeight ? 12 : 18;
        final double nameRowGap = useLogoWidthActionStack ? 6 : rowGap;
        final double logoLeftShift = useLogoWidthActionStack ? -8 : 0;
        final double bankingGap = isCompactHeight ? 12 : 22;
        final double bankingPanelPadding = isCompactHeight ? 12 : 18;
        final double scannerMinHeight = useCompactScannerCard ? 108 : (isCompactHeight ? 86 : 214);
        final double scannerTopInset = identityPanelPadding + 2;
        final double scannerBottomInset = scannerTopInset;
        final double scannerInnerPadding = useCompactScannerCard ? 10 : (isCompactHeight ? 8 : 18);
        final double scannerTopGap = useCompactScannerCard ? 6 : (isCompactHeight ? 6 : 26);
        final double scannerIconBox = useCompactScannerCard ? 44 : (isCompactHeight ? 32 : 64);
        final double scannerIconSize = useCompactScannerCard ? 26 : (isCompactHeight ? 18 : 36);
        final double bankingBaseWidth = isCompactHeight ? 286 : 352;
        final double bankingPulseScaleMax = _hasActiveBanking ? 1.036 : 1.2;
        final double gridGap = isCompactHeight ? 10 : 16;
        final double contentWidth = math.max(
          320,
          constraints.maxWidth - (outerCardSidePadding * 2),
        );
        final double gridColumnWidth = stackVertically
          ? contentWidth
          : (contentWidth - (gridGap * (gridColumns - 1))) / gridColumns;
        final double leftPanelWidth = stackVertically
          ? contentWidth
          : (gridColumnWidth * leftPanelColumns) + (gridGap * (leftPanelColumns - 1));
        final double rightPanelWidth = stackVertically
          ? contentWidth
          : (gridColumnWidth * rightPanelColumns) + (gridGap * (rightPanelColumns - 1));
        final double logoEdgeInsetFromCard = identityPanelPadding + logoLeftShift;
        final double stackedActionMaxWidth = math.max(
          logoSize,
          leftPanelWidth - (logoEdgeInsetFromCard * 2),
        );
        final double editProfileButtonWidth = useLogoWidthActionStack
          ? stackedActionMaxWidth
          : bankingBaseWidth * 1.2;
        final double bankingButtonWidth = useLogoWidthActionStack
          ? editProfileButtonWidth / bankingPulseScaleMax
          : bankingBaseWidth;
        final double scannerCardWidth = useCompactScannerCard
          ? editProfileButtonWidth
          : math.max(160, math.min(198, contentWidth * 0.5));
        final double scannerCardCollapsedHeight = useCompactScannerCard
          ? scannerCardWidth * 0.5
          : scannerCardWidth;
        final double scannerCardExpandedHeight = useCompactScannerCard
          ? scannerCardWidth
          : scannerCardWidth;

        final Widget identityPanel = Container(
          padding: EdgeInsets.all(identityPanelPadding),
          decoration: BoxDecoration(
          ...
          child: Column(
            ...
            children: [
                        ...
                        child: Stack(
                          ...
                          children: [
                            Container(
                              width: logoSize,
                              height: logoSize,
                              padding: EdgeInsets.all(logoBorderThickness),
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(28),
                                gradient: const LinearGradient(
                                ...
                                child: Container(
                                  ...
                                  child: _businessLogoUrl != null &&
                                      ...
                                      ? Image.network(
                                          ...
                                          errorBuilder: (context, error, stackTrace) {
                                            return _buildBusinessLogoFallback(
                                              iconSize: isCompactHeight ? 34 : 42,
                                            );
                                          },
                                        )
                                      : _buildBusinessLogoFallback(
                            ...
                            Positioned(
                              right: isCompactHeight ? -2 : -4,
                              bottom: isCompactHeight ? -2 : -4,
                              child: Container(
                      ...
                      child: Column(
                        ...
                        Text(
                          ...
                          style: theme.textTheme.headlineSmall?.copyWith(
                            ...
                            height: 1.05,
                            fontSize: useLogoWidthActionStack
                              ? 18
                                : (isCompactHeight ? 22 : null),
                          ),
                          maxLines: useLogoWidthActionStack ? 1 : null,
                          overflow: useLogoWidthActionStack ? TextOverflow.ellipsis : null,
                        ),
                        if (!useLogoWidthActionStack) ...[
                          SizedBox(height: isCompactHeight ? 6 : 10),
                          Text(
                            'Keep your storefront sharp, your payout setup healthy, and your member scans within easy reach.',
                            style: theme.textTheme.bodyMedium?.copyWith(
                              color: const Color(0xFF486581),
                              height: 1.45,
                              fontSize: isCompactHeight ? 13 : null,
                            ),
              ...
              SizedBox(height: useLogoWidthActionStack ? 4 : (isCompactHeight ? 10 : 14)),
              Align(
                alignment: useLogoWidthActionStack
                    ? Alignment.center
                    : Alignment.centerLeft,
                child: AnimatedContainer(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      foregroundColor: Colors.white,
                      padding: EdgeInsets.symmetric(
                        horizontal: useLogoWidthActionStack ? 10 : (isCompactHeight ? 16 : 18),
                        vertical: useLogoWidthActionStack ? 10 : (isCompactHeight ? 10 : 14),
                      ),
                      shape: RoundedRectangleBorder(
              ...
              SizedBox(height: useLogoWidthActionStack ? 6 : bankingGap),
              AnimatedBuilder(
                animation: _pulseAnimation,
                builder: (context, child) {
                  final double scale = _hasActiveBanking
                      ? 1 + ((_pulseAnimation.value - 1) * 0.18)
                  ...
                  child: AnimatedContainer(
                    ...
                    child: InkWell(
                      ...
                      child: AnimatedContainer(
                        ...
                        padding: EdgeInsets.all(bankingPanelPadding),
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(22),
                          gradient: LinearGradient(
                            colors: [
                              bankingColor.withValues(alpha: 0.18),
                              bankingColor.withValues(alpha: 0.06),
                          ...
                          border: Border.all(
                            color: bankingColor.withValues(alpha: 0.55),
                            width: 1.4,
                          ),
                          boxShadow: [
                            BoxShadow(
                              color: bankingColor.withValues(alpha: _hasActiveBanking ? 0.35 : 0.45),
                              blurRadius: _hasActiveBanking ? 24 : 30,
                              spreadRadius: _hasActiveBanking ? 1 : 2,
                              offset: const Offset(0, 10),
                            ),
                          ],
                        ),
                        child: useLogoWidthActionStack
                            ? Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Icon(
                            ...
                            : Row(
                                children: [
                                  Container(
                                    width: isCompactHeight ? 42 : 52,
                                    height: isCompactHeight ? 42 : 52,
                                    decoration: BoxDecoration(
                                      color: bankingColor.withValues(alpha: 0.14),
                                      borderRadius: BorderRadius.circular(18),
                                    ),
                                    child: Icon(
                                      bankingIcon,
                                      color: bankingColor,
                                      size: isCompactHeight ? 22 : 28,
                                    ),
                                  ),
        ...
        final Widget scannerPanel = Container(
          height: stackVertically ? null : double.infinity,
          padding: EdgeInsets.all(scannerPanelPadding),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(28),
            gradient: const LinearGradient(
          ...
          child: InkWell(
            ...
            child: Container(
              height: stackVertically ? null : double.infinity,
              constraints: stackVertically
                  ? BoxConstraints(minHeight: scannerMinHeight)
                  : null,
              decoration: useCompactScannerCard
                ? null
                : BoxDecoration(
                  borderRadius: BorderRadius.circular(24),
                  color: Colors.white.withValues(alpha: 0.1),
                ),
              padding: useCompactScannerCard
                ? EdgeInsets.zero
                : EdgeInsets.all(scannerInnerPadding),
              child: useCompactScannerCard
                  ? Center(
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 220),
                        curve: Curves.easeOutCubic,
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: theme.textTheme.headlineSmall?.copyWith(
                            color: Colors.white,
                            fontWeight: FontWeight.w900,
                            fontSize: isCompactHeight ? 14 : null,
                          ),
                        ),
                        SizedBox(height: isCompactHeight ? 4 : 10),
                        Text(
                          _isMobilePlatform
                              ? 'Tap to open the member scanner now.'
                              : 'Tap for scanner guidance. Camera scanning stays in the mobile app.',
                          style: theme.textTheme.bodyMedium?.copyWith(
                            color: Colors.white.withValues(alpha: 0.88),
                            height: 1.4,
                            fontSize: isCompactHeight ? 10 : null,
                          ),
                          maxLines: isCompactHeight ? 2 : null,
                          overflow: isCompactHeight ? TextOverflow.ellipsis : null,
                        ),
                        ...
                        Row(
                          children: [
                            Container(
                              padding: EdgeInsets.symmetric(
                                horizontal: isCompactHeight ? 8 : 14,
                              ...
                              child: Text(
                                _isMobilePlatform ? 'Open Scanner' : 'Mobile Only',
                                style: const TextStyle(
        ...
        final Widget heroShell = Container(
          padding: EdgeInsets.fromLTRB(
            outerCardSidePadding,
            outerCardTopPadding,
            outerCardSidePadding,
            outerCardBottomPadding,
          ),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(32),
            gradient: const LinearGradient(
              colors: [Color(0xFFFFFCF7), Color(0xFFFFFEFB), Color(0xFFFDFBF8)],
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
            ),
            border: Border.all(
              color: const Color(0xFFE7D9C7).withValues(alpha: 0.82),
              width: 1.4,
            ),
            boxShadow: [
              BoxShadow(
                ...
                child: stackVertically
                    ? Column(
                        children: [
                          identityPanel,
                          SizedBox(
                            height: useCompactScannerCard
                              ? 0
                                : (isCompactHeight ? 8 : 20),
                          ),
                          useCompactScannerCard
                              ? Align(
                                  alignment: Alignment.center,
                                  child: AnimatedContainer(
                                    duration: const Duration(milliseconds: 220),
                              ...
                              : scannerPanel,
                          if (useCompactScannerCard)
                            const SizedBox(height: 2),
                        ],
                      )
                    : IntrinsicHeight(
                        key: const ValueKey('hero-layout-split'),
                        child: Row(
  ...
  Widget _buildScannerShutter() {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final double scannerViewportHeight = math.max(
          160,
          math.min(220, MediaQuery.of(context).size.width - 80),
        );
        return Container(
