-- Fix: admin_create_trusted_partner should also create a businesses row
-- Issue: Trigger only creates profiles + memberships + trusted_partners,
--        but NOT a businesses row, so the partner list shows person name instead of business name
...
BEGIN
  ...
  p_business_name := coalesce(p_metadata->>'business_name', '');
  p_category := coalesce(p_metadata->>'category', '');
  p_city := coalesce(p_metadata->>'city', '');
  p_contact := coalesce(p_metadata->>'contact', '');
  ...
  -- ========================================================================
  -- FIX: Create businesses row so the partner list shows the business name
  -- The trigger only creates profiles + memberships + trusted_partners
  -- ========================================================================
  IF p_business_name <> '' THEN
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Log but don't fail - profile and auth are already created
      RAISE WARNING 'Failed to create business for TP %: %', v_user_id, SQLERRM;
    END;
  END IF;
