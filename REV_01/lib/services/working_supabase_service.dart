...
class SupabaseService {
  ...
  Future<AuthResponse> signIn({
    ...
    _logger.i('🔐 SupabaseService: Attempting sign in for $email');
    _logger.d(
    ...
    try {
      ...
      _logger.i(
        '🔐 SupabaseService: Sign in successful for $email, user: ${response.user?.id}',
      );
      return response;
    } catch (e) {
      ...
      if (e.toString().contains('token') || e.toString().contains('grant')) {
        _logger.e('🔐 SupabaseService: Token grant error during sign in: $e');
        throw Exception(
  ...
  /// Fetch the role for the current user from the `profiles` table.
  /// Returns the role string (e.g., 'member', 'merchant', 'admin') or null.
  ...
  Future<String?> getUserRole({String? userId}) async {
    ...
    try {
      // Prefer the memberships table (memberships.user_id -> role).
      try {
      ...
      // Fallback: try the profiles table (profiles.id -> role).
      try {
  ...
  /// Returns true if the resolved role for [userId] (or current user when
  /// omitted) equals 'merchant' (case-insensitive). Useful for quick guards.
  /// Merchants don't require subscription payments.
  Future<bool> isMerchant({String? userId}) async {
    final role = await getUserRole(userId: userId);
    final result = (role ?? '').toLowerCase() == 'merchant';
    _logger.d(
      'SupabaseService.isMerchant: userId=$userId, role="$role", isMerchant=$result',
    );
    return result;
  }
  ...
  void _initAuthStateListener() {
    _logger.d('🔐 SupabaseService: Setting up auth state change listener');
    client.auth.onAuthStateChange.listen(
      (AuthState authState) {
        _logger.i('🔐 SupabaseService: Auth state changed: ${authState.event}');
        _logger.d(
        ...
        switch (authState.event) {
          ...
          case AuthChangeEvent.userDeleted:
            _logger.i('🔐 SupabaseService: User deleted');
            break;
          default:
            _logger.w(
        ...
        if (authState.session == null &&
          ...
          _logger.w(
            '🔐 SupabaseService: This might indicate a token grant error',
          );
        }
      ...
      if (userType == null && currentUser != null) {
        try {
          ...
          if (existingProfile != null && existingProfile['role'] != null) {
            ...
            userType = existingRole == 'merchant' ? 'merchant' : 'user';
            _logger.i(
              '🔐 SupabaseService.createUserProfile: Using existing role from trigger: $existingRole',
            );
          }
        } catch (e) {
          _logger.w(
            '🔐 SupabaseService.createUserProfile: Could not read existing profile: $e',
          );
        }
      }

      // Default to 'user' if still not found
      userType ??= 'user';
      final role = userType == 'merchant' ? 'merchant' : 'user';

      _logger.i(
        '🔐 SupabaseService.createUserProfile: User type from metadata: $userType, assigned role: $role',
      );
  ...
  /// Authorize a user as a merchant by inserting/upserting a row in the
  /// `memberships` table (user_id -> role = 'merchant') and attempting to
  /// set the `profiles.role` to 'merchant' as a best-effort step. Returns
  ...
  Future<bool> authorizeMerchant({required String userId}) async {
    try {
      final row = {'user_id': userId, 'role': 'merchant'};

      // Try to upsert into memberships so repeated attempts don't fail.
      try {
        await client.from('memberships').upsert(row).select();
        _logger.i('authorizeMerchant: memberships row upserted for $userId');
      } catch (e) {
        _logger.e('authorizeMerchant: could not upsert memberships: $e');
      }
      ...
      try {
        ...
        _logger.i('authorizeMerchant: profiles.role upserted for $userId');
      } catch (e) {
        _logger.e('authorizeMerchant: could not upsert profiles: $e');
      }

      return true;
    } catch (e) {
      _logger.e('authorizeMerchant failed: $e');
      return false;
    }
  }

  // Store merchant data after signup
  Future<void> createMerchantProfile({
    required String userId,
    required Map<String, dynamic> merchantData,
  }) async {
    try {
      // Only include columns that exist in the current `merchants` table to
      ...
      final row = <String, dynamic>{
        ...
        if (merchantData.containsKey('created_at'))
      ...
      // Try to insert the merchant row. Use insert() so we don't accidentally
      ...
      if (inserted == null) {
        _logger.w('createMerchantProfile: insert returned null (no row)');
      } else {
        _logger.i('createMerchantProfile: merchant row created: $inserted');
      }
    } catch (e) {
      // Log the error but do not fail the overall signup flow. The user is
      // already created in Supabase Auth; merchant profile can be retried.
      _logger.e('Error creating merchant profile: $e');
  ...
  /// Calls a server-side RPC to complete merchant signup in one privileged
  /// operation. The RPC `complete_merchant_signup` should be implemented
  /// server-side as a SECURITY DEFINER function that upserts memberships,
  /// profiles and merchants (idempotent). `params` is a plain map of values
  ...
  Future<dynamic> completeMerchantSignup(Map<String, dynamic> params) async {
    try {
      // The server-side RPC is defined as complete_merchant_signup(payload jsonb)
      ...
      _logger.i('completeMerchantSignup: rpc returned: $res');
      return res;
    } on PostgrestException catch (e) {
      _logger.e('completeMerchantSignup PostgrestException: $e');
      rethrow;
    } catch (e) {
      _logger.e('completeMerchantSignup error: $e');
      rethrow;
    }
  }
  ...
  Future<void> debugUserProfile() async {
    try {
      ...
      if (response == null) {
        _logger.d('🔐 DEBUG: No profile data found for user');
        return;
      }

      _logger.d('🔐 DEBUG: Profile data found:');
      response.forEach((key, value) {
        _logger.d('🔐 DEBUG: $key = $value (${value?.runtimeType})');
      });
    } catch (e) {
      _logger.d('🔐 DEBUG: Error checking profile data: $e');
    }
  }
  ...
  Future<void> refreshSession() async {
    try {
      _logger.i('🔐 SupabaseService: Manually refreshing session tokens');
      final response = await client.auth.refreshSession();
      _logger.i('🔐 SupabaseService: Session refresh successful');
      _logger.i(
      ...
      _logger.e('🔐 SupabaseService: Session refresh failed: $e');
      if (e.toString().contains('token') || e.toString().contains('grant')) {
        _logger.e(
  ...
  Future<bool> isSessionValid() async {
    try {
      ...
      if (user == null || session == null) {
        _logger.w('🔐 SupabaseService: No active session');
        return false;
      }
      ...
      if (session.expiresAt != null) {
        ...
        _logger.d('🔐 SupabaseService: Session expires at: $expiresAt');
        _logger.d('🔐 SupabaseService: Current time: $now');
        _logger.d('🔐 SupabaseService: Session expired: $isExpired');
  ...
  Future<bool> checkConnectivity() async {
    try {
      ...
      _logger.w('🔐 SupabaseService: Connectivity check failed: $e');
