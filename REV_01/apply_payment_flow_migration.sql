-- Apply trusted partner payment flow migration
...
CREATE OR REPLACE FUNCTION update_trusted_partner_bank_accounts_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_trusted_partner_bank_accounts_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_trusted_partner_bank_accounts_updated_at();

CREATE OR REPLACE FUNCTION update_bill_approvals_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_bill_approvals_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_bill_approvals_updated_at();