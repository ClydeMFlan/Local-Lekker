// Send Deal Cancellation Email to Trusted Partner
//
// Called from the Flutter app when a member cancels an approved deal.
// Notifies the trusted partner by email so they know the slot is freed.
//
...
A member has cancelled their approved deal on Local Lekker.

Deal Details:
  Deal: ${deal_name}
  Amount: ${formattedAmount}
  Cancelled by: ${memberLabel}

No further action is required. The deal authorization has been closed.
...
`
      ...
      await client.send({
        from: smtpUser,
        to: tpEmail,
        subject: `Deal Cancelled – ${deal_name} (${formattedAmount})`,
        content: emailBody,
        html: htmlBody,
      })
      ...
      await client.close()
      emailSent = true
      console.log(`Deal cancellation email sent to ${tpEmail} for deal: ${deal_name}`)
    } catch (emailError) {
    ...
    console.error('send-deal-cancellation-email error:', error)
    return new Response(
