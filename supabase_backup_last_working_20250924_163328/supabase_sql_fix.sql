-- Fix for merchants table population issue
-- Execute this SQL in your Supabase SQL Editor (Dashboard > SQL Editor)

-- 1. Update the complete_business_profile function to populate merchants table
...
BEGIN
  ...
  INSERT INTO public.businesses (
    owner_user_id, name, category, address, latitude, longitude, contact_email, contact_number, verified
  ) VALUES (
...
-- 2. Update existing merchants records with business names from businesses table
UPDATE public.merchants
SET business_name = b.name,
    updated_at = NOW()
FROM public.businesses b
WHERE merchants.user_id = b.owner_user_id
  AND (merchants.business_name IS NULL OR merchants.business_name = '');