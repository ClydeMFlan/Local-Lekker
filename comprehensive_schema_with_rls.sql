...
-- =============================================================================
-- 1. MEMBERSHIPS TABLE - User role assignments
-- =============================================================================
...
CREATE TABLE public.profiles (
    ...
    surname TEXT,
    email TEXT,
    role TEXT DEFAULT 'user' CHECK (role IN ('member', 'trusted_partner', 'admin')),
    category TEXT,
    street TEXT,
    suburb TEXT,
    city TEXT,
...
CREATE POLICY "Members can view their own profile" ON public.profiles
    FOR SELECT USING (auth.uid() = id);

CREATE POLICY "Members can update their own profile" ON public.profiles
    FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Authenticated members can insert their profile" ON public.profiles
...
CREATE POLICY "Members can view their own trusted_partner record" ON public.trusted_partners
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Members can update their own trusted_partner record" ON public.trusted_partners
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Authenticated members can insert trusted_partner records" ON public.trusted_partners
...
CREATE POLICY "Members can view all businesses" ON public.businesses
...
CREATE POLICY "Authenticated members can create businesses" ON public.businesses
...
CREATE POLICY "Members can view all active discounts" ON public.trusted_partner_discounts
...
CREATE POLICY "Authenticated members can insert notifications" ON public.notifications
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated members can insert payments" ON public.payments
    FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
...
CREATE POLICY "Members can view their own QR codes" ON public.user_qr_codes
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Members can insert their own QR codes" ON public.user_qr_codes
...
WHERE schemaname = 'public'
    AND tablename IN (
        'memberships', 'profiles', 'trusted_partners', 'businesses',
        'trusted_partner_discounts', 'deal_authorizations', 'notifications',
        'processed_bills', 'payments', 'user_qr_codes', 'subscriptions',
