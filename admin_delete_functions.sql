...
-- Complete deletion with proper cascade and audit trails
-- =====================================================

-- =====================================================
-- FUNCTION: Delete Member Data (with audit preservation)
-- =====================================================
-- Deletes a member's data but NOT from auth.users
-- Auth user deletion must be done via Supabase Admin API
-- Member receipts are deleted, but TP's copy remains
...
BEGIN
  ...
  -- Delete member_receipts if column exists
  ...
  -- Delete deal_authorizations if column exists
  ...
  -- Delete generic tables that commonly reference the user with user_id
  FOR col_name IN SELECT unnest(ARRAY['user_qr_codes','subscriptions','payments','notifications','memberships']) LOOP
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name = col_name) THEN
      -- Only attempt delete if table has user_id column
      IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name = col_name AND column_name='user_id') THEN
        sql := format('DELETE FROM public.%I WHERE user_id = $1', col_name);
        EXECUTE sql USING member_user_id;
        deleted_count := deleted_count || jsonb_build_object(col_name || '_deleted', true);
      END IF;
    END IF;
  END LOOP;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='profiles' AND column_name='id') THEN
    EXECUTE 'DELETE FROM profiles WHERE id = $1' USING member_user_id;
...
-- =====================================================
-- FUNCTION: Delete Trusted Partner Data (with audit preservation)
-- =====================================================
-- Completely removes a trusted partner's data but NOT from auth.users
-- Auth user deletion must be done via Supabase Admin API
-- Removes all associated data including businesses, deals, receipts, and authorizations
...
AS $$
BEGIN
  RAISE NOTICE 'admin_delete_trusted_partner is deprecated. Use admin_delete_trusted_partner_data and Supabase Admin API';
...
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM admin_dashboard WHERE admin_user_id = user_id
  );
END;
$$;
...
-- Delete a member (preserves TP audit receipts):
-- SELECT admin_delete_member('member-uuid-here');

-- Delete a trusted partner (complete removal):
-- SELECT admin_delete_trusted_partner('tp-uuid-here');
